...
while True:
    ...
    while True:
        choice_yn = input("y/n : ")
        if choice_yn.lower() == "y":                # 방법 1
            break
        elif choice_yn == "n" or choice_yn == "N":  # 방법 2
            flag = 1    #주문 -> 결제창으로 이동
            break
        else:
            print("MSG: y 또는 n 만 입력해주세요.")
    # 7. 주문내역 출력!
    ...
    if flag == 1:
        ...
        for price in price_save:
            total_price += price
        print(f"MSG: 고객님이 주문하신 메뉴는 총 {len(menu_save)}개로 "
              f"총 결제금액은 {total_price}원 입니다.")
