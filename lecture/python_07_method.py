...
# ex) 아침마다 보고서를 만들려면 15줄의 코드를 작성 후 실행
#     -> 매일 아침 15줄 코드를 작성 및 실행(비효율적)
...
#     ex) print(), len(), type(), format(), list(), tuple(), 등
...
# 7. parameter와 return은 사용하지 않을 수 있음 ex) print()
#    (입력과 반환이 없는 함수도 존재)
...
# 2. global 키워드를 사용하는 방법(권장x, 사용금지)
a = 1
print(a)
def var_test():
    global a
...
#   - 인자(parameter): 함수 입력값
#   - 함수 입력값의 갯수가 함수를 호출 할 때마다 매번 다른경우
#     ->가변 길이 인자를 사용해야함!!
